...
public class ItemStackManager
{
    ...
    public void setkits()
    {
        ...
        for (String path : config.getConfigurationSection("kits").getKeys(false))
        {
            try
            {
                ...
                for (String pot : plugin.getConfig().getStringList(
                        "kits." + path + ".potion-effects"))
                {
                    ...
                    else
                    {
                        ...
                        potions.add(e.createEffect(dur.intValue(),
                                Integer.parseInt(poti[1])));
                    }
                }

                ItemStack helm = getItem(config.getString("kits." + path + ".helmet"),
                        false);
                ItemStack ches = getItem(
                        config.getString("kits." + path + ".chestplate"), false);
                ItemStack leg = getItem(config.getString("kits." + path + ".leggings"),
                        false);
                ItemStack boot = getItem(config.getString("kits." + path + ".boots"),
                        false);

                if (plugin.getConfig().getString("kits." + path + ".permission") != null
                        && !plugin.getConfig().getString("kits." + path + ".permission")
                                .equals("none"))
                    perm = plugin.getConfig().getString("kits." + path + ".permission");

                plugin.kit.kititems.put(path,
                        new KitEntry(stack.toArray(new ItemStack[0]), helm, boot, ches,
                                leg, perm, potions));
            }
            catch (Exception e)
            {
    ...
    public ItemStack itemStringToStack(String item, Integer amount)
    {
        ...
        if (itemArr.length > 1)
            return new ItemStack(Material.getMaterial(itemArr[0]), amount,
                    (itemArr[1].length() <= 3 ? Byte.parseByte(itemArr[1])
                            : ((short) Integer.parseInt(itemArr[1]))));
        return new ItemStack(Material.getMaterial(itemArr[0]), amount);
    }
}
