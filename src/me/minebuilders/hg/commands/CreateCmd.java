...
public class CreateCmd extends BaseCmd
{
    ...
    public boolean run()
    {
        ...
        {
            Util.msg(player, ChatColor.RED
                    + "You need to make a selection before making an arena!");
        }
        else
        {
            ...
            else
            {
                ...
                else if (Integer.parseInt(args[4]) % 30 != 0)
                {
                    player.sendMessage(ChatColor.RED
                            + "time-in-seconds must be divisible by 30!");
                ...
                {
                    player.sendMessage(ChatColor.RED
                            + "min-players cannot be more then max-players!");
                    sendHelpLine();
                }
                else
                {
                    ...
                    c.set("arenas." + args[1] + ".bound.world", player.getWorld()
                            .getName());
                    ...
                    c.set("arenas." + args[1] + ".info." + "timer",
                            Integer.parseInt(args[4]));
                    c.set("arenas." + args[1] + ".info." + "min-players",
                            Integer.parseInt(args[2]));
                    c.set("arenas." + args[1] + ".info." + "max-players",
                            Integer.parseInt(args[3]));
                    HG.arenaconfig.saveCustomConfig();
                    HG.arenaconfig.reloadCustomConfig();

                    Bound b = new Bound(player.getWorld().getName(), l.getBlockX(),
                            l.getBlockY(), l.getBlockZ(), l2.getBlockX(), l2.getBlockY(),
                            l2.getBlockZ());
                    HG.plugin.games.add(new Game(args[1], b, Integer.parseInt(args[4]),
                            Integer.parseInt(args[2]), Integer.parseInt(args[3]),
                            freeroam));
                    Util.msg(player, ChatColor.GREEN + "You created HungerGames arena "
                            + args[1] + "!");
                    return true;
                }
            }
