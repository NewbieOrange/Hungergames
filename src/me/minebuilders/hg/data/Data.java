...
public class Data
{
        ...
        if (new File(plugin.getDataFolder(), "arenas.yml").exists())
        {
            ...
            for (String s : arenadat.getConfigurationSection("arenas").getKeys(false))
            {
                ...
                try
                {
                    ...
                    minplayers = arenadat
                            .getInt("arenas." + s + ".info." + "min-players");
                    maxplayers = arenadat
                            .getInt("arenas." + s + ".info." + "max-players");
                }
                catch (Exception e)
                {
                ...
                try
                {
                    lobbysign = (Sign) getSLoc(
                            arenadat.getString("arenas." + s + "." + "lobbysign"))
                            .getBlock().getState();
                }
                catch (Exception e)
                {
                ...
                try
                {
                    for (String l : arenadat
                            .getStringList("arenas." + s + "." + "spawns"))
                    {
                ...
                {
                    b = new Bound(
                            arenadat.getString("arenas." + s + ".bound." + "world"), BC(
                                    s, "x"), BC(s, "y"), BC(s, "z"), BC(s, "x2"), BC(s,
                                    "y2"), BC(s, "z2"));
                }
                catch (Exception e)
                {
                ...
                plugin.games.add(new Game(s, b, spawns, lobbysign, timer, minplayers,
                        maxplayers, freeroam, isReady));
            }
        }
    }
    ...
    {
        String[] h = s.split(":");
        return new Location(Bukkit.getServer().getWorld(h[0]),
                Integer.parseInt(h[1]) + 0.5, Integer.parseInt(h[2]),
    ...
    {
        String[] h = s.split(":");
        return new Location(Bukkit.getServer().getWorld(h[0]), Integer.parseInt(h[1]),
