                ...
                && !st[0].equalsIgnoreCase("/login"))
        {
            if (st[0].equalsIgnoreCase("/hg"))
            {
                if (st.length >= 2
                        ...
                        && plugin.players.get(player.getName()).getGame().getStatus() == Status.RUNNING)
                {
                    event.setMessage("/");
                    event.setCancelled(true);
                    Util.msg(player, ChatColor.RED
                            + "You can't choose a kit while the game is running!");
                }
                return;
            }
            event.setMessage("/");
            event.setCancelled(true);
            Util.msg(player, ChatColor.RED
                    + "You can not use non-hungergames commands while ingame!");
        }
            ...
            if (p != null)
            {
                if (plugin.players.containsKey(p.getName()))
                {
                    player.sendMessage(ChatColor.RED
                            + "This player is currently playing Hungergames!");
                    event.setMessage("/");
                    event.setCancelled(true);
                }
            }
